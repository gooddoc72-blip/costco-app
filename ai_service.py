...
_RECEIPT_SYSTEM = (
    ...
    "- ⚠️ **수량 읽기**: 코스트코 영수증은 `995554  KS초콜릿아몬드  2x 18,395  36,790 T`\n"
    "  처럼 **금액 왼쪽에 `Nx`(또는 `N @`)로 개수**가 붙는다. 그 N이 수량이다.\n"
    "  · `2x 18,395  36,790` → 수량 2, 단가 18395, 금액 36790\n"
    "  · 개수 표시가 없으면 수량 1이고 단가=금액이다.\n"
    "  · **답하기 전에 수량 × 단가 = 금액 인지 반드시 계산해 볼 것.** 안 맞으면 개수를\n"
    "    놓친 것이다. 특히 수량을 1로 적었는데 금액이 단가보다 크면 개수를 못 본 것이니\n"
    "    다시 세어라. 수량을 1로 뭉개면 단가가 배로 뛰어 잘못 청구된다.\n"
