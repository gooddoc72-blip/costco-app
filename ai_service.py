...
_RECEIPT_SYSTEM = (
    ...
    "- ⚠️ **영수증 줄 구조 — 가장 중요**: 코스트코 코리아 영수증은 품목 하나가 **두 줄**이다.\n"
    "  첫 줄은 상품명, 둘째 줄은 `상품번호  수량  단가  금액 T` 다.\n"
    "  ```\n"
    "  KS종이타월\n"
    "  1768085     1     26,490      26,490 T\n"
    "  이디야골드블렌드\n"
    "  667538      3     41,990     125,970 T\n"
    "  ```\n"
    "  → 상품번호 1768085 / 상품명 KS종이타월 / 수량 1 / 단가 26490 / 금액 26490\n"
    "  → 상품번호 667538 / 상품명 이디야골드블렌드 / 수량 3 / 단가 41990 / 금액 125970\n"
    "  **수량은 상품번호 바로 오른쪽의 맨 앞 숫자**다. `2x` 같은 표시가 아니라 그냥 숫자다.\n"
    "  같은 상품이 **따로 두 줄**로 찍히기도 한다(각각 수량 1). 합치지 말고 **줄마다 그대로** 넣어라.\n"
    "- ⚠️ **할인(쿠폰) 줄**: `*** CPN` 이라는 줄이 나오면 **그 다음 두 줄이 할인**이다.\n"
    "  ```\n"
    "  꽃담초유연제5.7L\n"
    "  617031      1     16,990      16,990 T   ← 품목\n"
    "  *** CPN\n"
    "  꽃담초유연제5.7L\n"
    "  5234        1      3,500     - 3,500 T   ← 할인 3,500원\n"
    "  ```\n"
    "  · **마이너스는 금액 '앞'에 붙는다**(`- 3,500 T`). 금액이 음수면 무조건 할인이다.\n"
    "  · 할인 줄의 앞 숫자(5234)는 **상품번호가 아니라 쿠폰코드**다. 이걸 상품번호로 넣으면 있지도 않은 상품이 생긴다. 절대 items에 넣지 마라.\n"
    "  · 할인 줄은 **바로 위 품목의 discount에 양수로** 넣는다. 상품명이 위 품목과 같거나 뒤에 IRC/CPN 이 붙는다.\n"
    "  · 그래서 item_kinds(품목 종수)와 total_qty(총수량)에 할인 줄은 세지 않는다.\n"
    "- 검산(반드시 계산해 보고 답할 것):\n"
    "  · 줄마다 **수량 × 단가 = 금액**. 안 맞으면 수량을 놓친 것이다.\n"
    "  · (품목 금액 합계) − (할인 합계) = **'합계 (VAT 포함)'** 금액.\n"
    "  · 할인 합계 = 영수증 하단 **'쿠폰합계'**.\n"
    "  · 수량 합계 = 영수증 하단 **'총 판매 상품 수'**.\n"
    "  안 맞으면 줄을 다시 세어라 — 특히 같은 상품의 두 번째 줄을 빠뜨렸는지 확인.\n"
    "- coupon_total: 영수증 하단 **'쿠폰합계'**(또는 COUPON TOTAL) 옆의 **할인 합계 금액**(개수 아님). 예 `쿠폰합계 62,000` → 62000. 없으면 0.\n"
    ...
    "- total_qty: 영수증 하단 **'총 판매 상품 수'**(또는 총수량/수량합계). 없으면 품목 수량의 합.\n"
    "- item_kinds: 품목 종수(줄 수).\n"
    "- total_amount: **'합계 (VAT 포함)'** 옆의 최종 결제금액(할인 반영 후). 숫자만.\n"
...
def repair_receipt_math(data, tolerance=1):
    ...
    for _i in (data.get("items") or []):
        ...
        if not (_q and _a) or abs(_q * _u - _a) <= tolerance:
            continue
        # 수량 1로 읽었는데 금액이 단가로 딱 나눠떨어지면 개수를 놓친 것이다.
        # (`2x 18,395  36,790` 을 1개 36,790원으로 뭉개는 오독이 실제로 있었다)
        # 이걸 놓치면 단가가 배로 뛰어 그 사용자에게 두 배로 청구된다.
        if _q == 1 and _u and _a > _u and _a % _u == 0:
            _i["수량"] = _a // _u
            _n += 1
        elif _a % _q == 0:
            _i["단가"] = _a // _q
