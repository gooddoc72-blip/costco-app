...
def render(USERNAME: str, IS_ADMIN: bool, settings: dict):
    ...
    st.divider()
    st.subheader("📅 정산 이력")
    _get_dates = _cached_saved_dates if _cached_saved_dates else get_saved_dates
    _get_hist  = _cached_daily_orders if _cached_daily_orders else get_daily_orders
    saved_dates_list = _get_dates(USERNAME)
    if saved_dates_list:
        _hd_col, _del_col = st.columns([2, 1])
        sel_date = _hd_col.selectbox("날짜 선택", saved_dates_list, key="profit_hist_date")
        _del_col.markdown('<div style="height:28px"></div>', unsafe_allow_html=True)
        _do_delete = _del_col.button(f"🗑 {sel_date} 삭제", key="del_hist_date", use_container_width=True)

        if sel_date:
            hist_orders = _get_hist(USERNAME, sel_date)
            if hist_orders:
                _TH_H  = "text-align:{a};padding:4px 6px;font-size:12px;color:#444;background:#fafafa;border-bottom:2px solid #dee2e6"
                _CH    = "padding:5px 6px;font-size:13px;border-bottom:1px solid #f0f0f0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;vertical-align:middle"
                _CH_NM = "padding:5px 6px;font-size:13px;border-bottom:1px solid #f0f0f0;white-space:normal;word-break:break-all;line-height:1.4;vertical-align:middle"

                st.caption("💡 **구입가** 열을 수정하면 수입이 즉시 재계산됩니다. 저장하면 제품 DB도 업데이트됩니다.")

                # 헤더
                _hdr_c, _cost_hdr_c = st.columns([10, 2])
                _hdr_c.markdown(
                    '<table style="width:100%;border-collapse:collapse;table-layout:fixed">'
                    '<thead><tr>'
                    f'<th style="width:10%;{_TH_H.format(a="left")}">수취인</th>'
                    f'<th style="width:36%;{_TH_H.format(a="left")}">상품명</th>'
                    f'<th style="width:14%;{_TH_H.format(a="left")}">옵션</th>'
                    f'<th style="width:5%;{_TH_H.format(a="center")}">수량</th>'
                    f'<th style="width:13%;{_TH_H.format(a="right")}">정산예정</th>'
                    f'<th style="width:11%;{_TH_H.format(a="right")}">고객택배비</th>'
                    f'<th style="width:11%;{_TH_H.format(a="right")}">💰 수입</th>'
                    '</tr></thead></table>',
                    unsafe_allow_html=True
                )
                _cost_hdr_c.markdown("<b style='font-size:13px;color:#444'>구입가✏️</b>", unsafe_allow_html=True)

                # 행별 표시 (구입가만 number_input, 나머지 HTML)
                _hcpfx = f"hcost_{sel_date}_"
                _hist_live_costs = {}

                for _hi, _ho in enumerate(hist_orders):
                    _def_cost  = int(_ho.get('cost_price') or 0)
                    _cur_cost  = int(st.session_state.get(f"{_hcpfx}{_hi}", _def_cost) or 0)
                    _settle    = int(_ho.get('settlement') or 0)
                    _cfee      = int(_ho.get('shipping_fee') or 0)
                    _dcost     = int(_ho.get('delivery_cost') or shipping_cost)
                    _bcost     = int(_ho.get('box_cost') or box_cost)
                    _pv        = (_settle + _cfee) - (_cur_cost + _dcost + _bcost)
                    _pv_color  = '#E74C3C' if _pv < 0 else '#1D9E75'

                    _row_c, _cost_c = st.columns([10, 2])
                    _row_c.markdown(
                        f'<table style="width:100%;border-collapse:collapse;table-layout:fixed">'
                        f'<tr>'
                        f'<td style="{_CH};width:10%">{_ho.get("recipient","")}</td>'
                        f'<td style="{_CH_NM};width:36%">{_ho.get("product_name","")}</td>'
                        f'<td style="{_CH};width:14%;color:#666;font-size:12px">{str(_ho.get("option_info") or "")[:25]}</td>'
                        f'<td style="{_CH};width:5%;text-align:center">{int(_ho.get("qty") or 1)}</td>'
                        f'<td style="{_CH};width:13%;text-align:right">{fmt(_settle)}</td>'
                        f'<td style="{_CH};width:11%;text-align:right">{fmt(_cfee)}</td>'
                        f'<td style="{_CH};width:11%;text-align:right;font-weight:700;color:{_pv_color}">{fmt(_pv)}</td>'
                        f'</tr></table>',
                        unsafe_allow_html=True
                    )
                    _new_cost = _cost_c.number_input(
                        "", value=_def_cost, min_value=0, step=100,
                        label_visibility="collapsed",
                        key=f"{_hcpfx}{_hi}"
                    )
                    _hist_live_costs[_hi] = _new_cost

                # 실시간 순수익
                _total_live = sum(
                    (int(hist_orders[_hi].get('settlement') or 0) + int(hist_orders[_hi].get('shipping_fee') or 0))
                    - (_hist_live_costs.get(_hi, int(hist_orders[_hi].get('cost_price') or 0))
                       + int(hist_orders[_hi].get('delivery_cost') or shipping_cost)
                       + int(hist_orders[_hi].get('box_cost') or box_cost))
                    for _hi in range(len(hist_orders))
                )
                _total_orig = sum(int(o.get('profit') or 0) for o in hist_orders)
                _delta_val  = _total_live - _total_orig
                _live_col, _ = st.columns([2, 4])
                _live_col.metric(
                    "💰 현재 순수익",
                    f"{fmt(_total_live)}원",
                    delta=f"{'+' if _delta_val >= 0 else ''}{fmt(_delta_val)}원" if _delta_val != 0 else None,
                )

                # 저장 버튼
                _hc_s, _ = st.columns([2, 4])
                if _hc_s.button("💾 구입가 저장 + 제품DB 업데이트", type="primary",
                                key="save_hist_cost", use_container_width=True):
                    conn = get_user_db(USERNAME)
                    _sc_row = conn.execute("SELECT value FROM settings WHERE key='shipping_cost'").fetchone()
                    _bc_row = conn.execute("SELECT value FROM settings WHERE key='box_cost'").fetchone()
                    _s_cost = int(_sc_row[0]) if _sc_row and _sc_row[0] else 1800
                    _b_cost = int(_bc_row[0]) if _bc_row and _bc_row[0] else 300
                    _updated = 0
                    _prod_updated = 0
                    now = datetime.now().strftime("%Y-%m-%d %H:%M")
                    for _i, _orig_row in enumerate(hist_orders):
                        _new_cost = int(_hist_live_costs.get(_i, int(_orig_row.get('cost_price') or 0)))
                        if _new_cost != int(_orig_row.get('cost_price') or 0):
                            _settle = int(_orig_row.get('settlement') or 0)
                            _ship   = int(_orig_row.get('shipping_fee') or 0)
                            _del_c  = int(_orig_row.get('delivery_cost') or _s_cost)
                            _box_c  = int(_orig_row.get('box_cost') or _b_cost)
                            _profit = (_settle + _ship) - (_new_cost + _del_c + _box_c)
                            conn.execute(
                                "UPDATE daily_orders SET cost_price=?, profit=?, matched=1 WHERE id=?",
                                (_new_cost, _profit, _orig_row['id'])
                            )
                            _updated += 1
                            _pno = str(_orig_row.get('product_no') or '').strip()
                            if _pno and _new_cost > 0:
                                conn.execute(
                                    "UPDATE products SET unit_price=?, updated_at=? WHERE product_no=?",
                                    (_new_cost, now, _pno)
                                )
                    ...
                    conn.close()
                    if _updated:
                        _msg = f"✅ {_updated}건 구입가 저장"
                        if _prod_updated:
                            _msg += f" + 제품DB {_prod_updated}개 단가 업데이트"
                        # 위젯 state 초기화 (저장 후 새 값으로 재로드)
                        for _ci in range(len(hist_orders)):
                            st.session_state.pop(f"{_hcpfx}{_ci}", None)
                        st.success(_msg)
                        st.rerun()
                    else:
                        st.info("변경된 구입가가 없습니다.")
                ...
                # ── 수익 마이너스 — 네이버 판매가 조정 ──
                _hloss = [(_hi, _ho) for _hi, _ho in enumerate(hist_orders)
                          if int(_ho.get('profit') or 0) < 0 and int(_ho.get('cost_price') or 0) > 0]
                if _hloss:
                    _hmargin = int(_gs('target_margin') or 10) / 100
                    with st.expander(f"🔴 수익 마이너스 {len(_hloss)}건 — 네이버 판매가 조정", expanded=False):
                        _pu_h = cached_user_products(USERNAME) if cached_user_products else []
                        _hloss_seen = {}
                        for _, _ho in _hloss:
                            _lkey = str(_ho.get('product_name') or '').strip()
                            if _lkey and _lkey not in _hloss_seen:
                                _hloss_seen[_lkey] = _ho
                        _hloss_apply = []
                        for _hli, (_hpname, _hrow) in enumerate(_hloss_seen.items()):
                            _hqty    = max(1, int(_hrow.get('qty') or 1))
                            _hsettle = int(_hrow.get('settlement') or 0)
                            _hcfee   = int(_hrow.get('shipping_fee') or 0)
                            _hcost   = int(_hrow.get('cost_price') or 0)
                            _hprofit = int(_hrow.get('profit') or 0)
                            _hunit_cost   = _hcost // _hqty
                            _hunit_settle = _hsettle // _hqty
                            _hcur_sale    = max(100, int(_hunit_settle / 0.945 / 100) * 100)
                            _hmin_needed  = _hunit_cost + shipping_cost + box_cost - _hcfee / _hqty
                            _hsugg = max(int(_hmin_needed * (1 + _hmargin) / 0.945 / 100) * 100,
                                         _hcur_sale + 100)
                            # 네이버 상품정보 조회
                            _hup = next((p for p in (_pu_h or [])
                                         if p.get('match_keyword') == _hpname
                                         or p.get('costco_name', '').strip() == _hpname.strip()), None)
                            _hnv_pno  = (_hup or {}).get('naver_origin_pno', '') or ''
                            _h_is_nv  = int((_hup or {}).get('from_naver') or 0) == 1
                            _hnv_name = (_hup.get('costco_name', '') if _hup and _h_is_nv else '') or ''
                            _hdisp    = _hnv_name if _hnv_name else _hpname

                            st.markdown(f"##### 🔴 {_hdisp[:60]}")
                            # 현황 카드
                            _hcard = (
                                '<div style="display:flex;gap:6px;margin-bottom:10px;flex-wrap:wrap">'
                                f'<div style="flex:1;min-width:85px;background:#fff3f3;border:1px solid #fcc;'
                                f'border-radius:6px;padding:7px 8px;text-align:center">'
                                f'<div style="font-size:11px;color:#888">현재 판매가</div>'
                                f'<div style="font-size:14px;font-weight:700;color:#333">{fmt(_hcur_sale)}원</div></div>'
                                f'<div style="flex:1;min-width:85px;background:#f8f8f8;border:1px solid #eee;'
                                f'border-radius:6px;padding:7px 8px;text-align:center">'
                                f'<div style="font-size:11px;color:#888">정산금액</div>'
                                f'<div style="font-size:14px;font-weight:600">{fmt(_hsettle)}원</div></div>'
                                f'<div style="flex:1;min-width:85px;background:#f8f8f8;border:1px solid #eee;'
                                f'border-radius:6px;padding:7px 8px;text-align:center">'
                                f'<div style="font-size:11px;color:#888">고객택배비</div>'
                                f'<div style="font-size:14px;font-weight:600">{fmt(_hcfee)}원</div></div>'
                                f'<div style="flex:1;min-width:85px;background:#f8f8f8;border:1px solid #eee;'
                                f'border-radius:6px;padding:7px 8px;text-align:center">'
                                f'<div style="font-size:11px;color:#888">구매가격</div>'
                                f'<div style="font-size:14px;font-weight:600">{fmt(_hcost)}원</div></div>'
                                f'<div style="flex:1;min-width:85px;background:#ffe0e0;border:1px solid #faa;'
                                f'border-radius:6px;padding:7px 8px;text-align:center">'
                                f'<div style="font-size:11px;color:#888">현재 수익</div>'
                                f'<div style="font-size:14px;font-weight:700;color:#E74C3C">{fmt(_hprofit)}원</div></div>'
                                f'</div>'
                            )
                            st.markdown(_hcard, unsafe_allow_html=True)
                            _hca, _hcb, _hcd, _hcc = st.columns([1, 3, 2, 1])
                            _hdo = _hca.checkbox("적용", value=True, key=f"hlp_chk_{_hli}")
                            _hnew_price = _hcb.number_input(
                                "🔧 수정 판매가 (원)",
                                value=_hsugg, min_value=100, step=100,
                                key=f"hlp_price_{_hli}"
                            )
                            _hnew_cfee = _hcd.number_input(
                                "🔧 수정 택배비 (원)",
                                value=int(_hcfee), min_value=0, step=100,
                                key=f"hlp_cfee_{_hli}"
                            )
                            _hnew_settle = int(_hnew_price * 0.945)
                            _hnew_profit = (_hnew_settle * _hqty + _hnew_cfee) - (_hcost + shipping_cost + box_cost)
                            if _hnew_profit < 0:
                                _hcc.error(f"❌ {fmt(_hnew_profit)}원")
                            else:
                                _hcc.success(f"✅ +{fmt(_hnew_profit)}원")
                            _hpno_lbl = "✅ 네이버 상품번호" if _hnv_pno else "⚠️ 네이버 상품번호 (미입력)"
                            _hpno = st.text_input(
                                _hpno_lbl,
                                value=_hnv_pno, key=f"hlp_pno_{_hli}",
                                placeholder="네이버 originProductNo"
                            )
                            if _hdo:
                                _hloss_apply.append({
                                    'name': _hpname, 'display_name': _hdisp,
                                    'new_sale_price': _hnew_price,
                                    'new_shipping_fee': _hnew_cfee,
                                    'product_no': _hpno, 'new_profit': _hnew_profit,
                                })
                            st.divider()
                        if _hloss_apply:
                            _still_neg = [t for t in _hloss_apply if t['new_profit'] < 0]
                            if _still_neg:
                                st.warning(f"⚠️ 여전히 마이너스 {len(_still_neg)}건 — 판매가를 더 올려주세요.")
                            if HAS_NAVER_API and st.button(
                                "✅ 선택 상품 네이버 판매가 적용", type="primary",
                                key="hloss_naver_apply", use_container_width=True
                            ):
                                if not api_id or not api_secret:
                                    st.error("설정 탭에서 네이버 API 키를 등록해주세요.")
                                else:
                                    _hok, _hfail = [], []
                                    for t in _hloss_apply:
                                        if not t['product_no']:
                                            _hfail.append(f"{t['display_name'][:20]}: 상품번호 미입력")
                                            continue
                                        _r_ok, _r_err = naver_api.update_product_price(
                                            api_id, api_secret, t['product_no'], t['new_sale_price']
                                        )
                                        (_hok if _r_ok else _hfail).append(
                                            t['display_name'] if _r_ok
                                            else f"{t['display_name'][:20]}: {_r_err}"
                                        )
                                    if _hok:
                                        st.success(f"✅ 적용 완료: {', '.join(_hok)}")
                                    for _hf in _hfail:
                                        st.error(f"❌ {_hf}")
