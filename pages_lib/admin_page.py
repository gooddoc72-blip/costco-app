...
def render(USERNAME: str, IS_ADMIN: bool, settings: dict):
    ...
    for u in users:
        ...
        with st.expander(f"{role} {u['display_name']} ({u['username']}) — {status_txt} | {u['created_at']}"):
            if not u['is_admin']:
                ...
                # ── 🛍 네이버 등록 한도(누적) — 사용자 토큰으로 올라간 총량 ──
                #   카페24 한도와 별개다. 카페24 한도는 '관리자 카탈로그 사용량',
                #   이건 '그 사용자 스토어에 이 프로그램으로 올린 상품 수'(수동·무인·
                #   대행 전 경로 합산). 열어준 기능과 무관하게 누구에게나 적용된다.
                from db_naver_reg import naver_reg_quota
                _nq = naver_reg_quota(u['username'])
                _nc1, _nc2 = st.columns([1, 2])
                _nlim_new = _nc1.number_input(
                    "네이버 등록 한도 (누적, 0=무제한)", min_value=0, max_value=1000000,
                    step=50, value=int(_nq['limit'] or 0),
                    key=f"nvlim_{u['username']}",
                    help="이 사용자의 네이버 커머스 API 토큰으로 등록할 수 있는 누적 "
                         "상품 수. 수동등록·무인자동·카페24 대행을 모두 합산합니다. "
                         "0이면 제한 없음.")
                with _nc2:
                    st.write("")
                    if _nq['limit']:
                        st.caption(f"현재 **{_nq['used']} / {_nq['limit']}**개 등록 "
                                   f"(남은 {_nq['remaining']}개)"
                                   + ("  ·  🚫 한도 소진" if _nq['blocked'] else ""))
                    else:
                        st.caption(f"현재 누적 등록 **{_nq['used']}개** · 한도 없음")
                if int(_nlim_new) != int(_nq['limit'] or 0):
                    from db_naver_reg import set_naver_reg_limit
                    set_naver_reg_limit(u['username'], int(_nlim_new))
                    st.success(f"✅ 네이버 등록 한도 {int(_nlim_new)}개로 저장"
                ...
                _sp_new = st.checkbox(
                    "🏪 직접구매 계정 (자기 돈으로 직접 삼 — 청구서를 만들지 않음)",
                    value=_sp_cur, key=f"selfpur_{u['username']}",
                    help="직접 매장에서 사는 계정에 켜세요. 켜도 영수증 매칭과 재고 "
                         "정리는 그대로 됩니다(주문 구입가·재고 차감의 근거). "
                         "다만 자기 돈으로 산 것이라 청구서를 만들지 않아 "
                         "정산리스트·미입금자에 나타나지 않습니다.")
                if _sp_new != _sp_cur:
                    set_setting(u['username'], 'self_purchase', '1' if _sp_new else '')
                    st.success("🏪 직접구매 계정 — 매칭·재고는 그대로, 청구서만 안 만듭니다"
                               if _sp_new else
                               "✅ 구매대행 계정으로 되돌림 — 이제 청구서가 만들어집니다")
                    st.rerun()
                if _sp_cur:
    ...
    def _render_combined_print(_subs, _period_label):
        ...
        _compact = _cc2.checkbox("작게(2단)", value=False, key="shop_combo_2col",
                                 help="목록을 두 단으로 흘려 더 많이 담습니다. "
                                      "종수가 적은 날에 유리합니다.")
        _hide_money = _cc3.checkbox("금액 숨기기", value=False, key="shop_combo_nomoney",
        ...
        _pc2.download_button("📥 합본 HTML 저장", data=_html.encode('utf-8'),
                             ...
                             mime="text/html", key="dl_shop_combo")
    ...
    if _subs_sel:
        ...
        st.divider()
        _render_combined_print(_subs_sel, _period_label)
        st.divider()
