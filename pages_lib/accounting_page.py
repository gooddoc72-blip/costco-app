...
def render(USERNAME: str):
    ...
    with _tab1:
        ...
        _net = pl['operating_profit'] - _adcost
        _k1, _k2, _k3, _k4 = st.columns(4)
        _k1.metric("매출액", f"{fmt(pl['sales'])}원", delta=f"{pl['cnt']}건")
        _k2.metric("매출원가", f"{fmt(pl['cost'])}원")
        _k3.metric("매출총이익", f"{fmt(pl['gross_profit'])}원")
        _k4.metric("💵 순이익(영업이익−광고비)", f"{fmt(_net)}원")

        _rows = [
            ("Ⅰ. 매출액", pl['sales'], "고객 결제 총액(주문금액)"),
            ("Ⅱ. 매출원가", -pl['cost'], "매입가(코스트코 등)"),
            ("　매출총이익", pl['gross_profit'], "매출 − 매출원가"),
            ("Ⅲ. 판매관리비", -(pl['delivery'] + pl['box'] + pl['commission'] + _adcost), ""),
            ("　└ 지급수수료(플랫폼)", -pl['commission'], "총매출+배송비−정산수령(추정)"),
            ("　└ 운반비(택배)", -pl['delivery'], ""),
            ("　└ 포장비", -pl['box'], ""),
            ("　└ 광고선전비", -_adcost, f"{_ad_month} 광고비(수동입력분)"),
            ("Ⅳ. 영업이익(순이익)", _net, "매출총이익 − 판관비"),
        ]
        _html = '<table style="width:100%;border-collapse:collapse;font-size:14px">'
        for _name, _amt, _memo in _rows:
            _bold = _name.startswith(("Ⅰ", "Ⅱ", "Ⅲ", "Ⅳ", "　매출총", "Ⅳ"))
        ...
        st.caption("⚠️ 부가세 구분(과세/면세)·매입세액공제는 Phase 2에서 정확 반영됩니다. "
                   "현재 순이익은 광고비(수동)까지 차감한 관리용 추정입니다.")
